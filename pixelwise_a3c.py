...
class PixelWiseA3C_InnerState():
    ...
    def update(self, statevar):
        ...
        if statevar is None:
            R = torch.zeros(self.batch_size, 3, 256, 256).cuda()
        else:
        ...
        print('pi_loss:'+str(pi_loss.mean())+'   v_loss:'+str(v_loss.mean()))
    ...
    def act_and_train(self, state, reward):
        ...
        if np.nan in p_trans.detach().cpu().numpy():
            print(statevar)
        ...
        return action.squeeze(1).detach().cpu().numpy()
