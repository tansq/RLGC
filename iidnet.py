...
class Global_Local_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask_index = torch.nonzero((mask == 1), as_tuple=False)[:, 1].view(tmp.size(0), -1)
        ...
        for t in range(1, dy_T):
            ...
            mask_index_top = torch.nonzero((mask == 1), as_tuple=False)[:, 1].view(tmp.size(0), -1).gather(1, indexes[:, :, t])
...
class InpaintingForensics():
    ...
    def test(self):
        ...
        for cnt, items in enumerate(self.test_loader):
            print(cnt, end='\r')
            Ii, Mg = (item.cuda() for item in items[:-1])
            Ii = Variable(Ii.cuda(), requires_grad=True)
            Ii.retain_grad()
            filename = items[-1][0]
            Mo, gen_loss = self.giid_model.process(Ii, Mg)
            self.giid_model.backward(gen_loss)
            adversaries = torch.clamp(Ii+0.1*torch.sign(Ii.grad*Mg), -1.0, 1.0)
            B, H, W, C = adversaries.shape
            for count in range(B):
                adv = adversaries[count]* 127.5 + 127.5
                #adv = adversaries[count]* 255.0
                adv = adv.permute(1, 2, 0).cpu().detach().numpy()
                cv2.imwrite('a-adversary/adversary_' + str(idx*B+count).zfill(5) + '.png', adv)
                print('a-adversary/adversary_' + str(idx*B+count).zfill(5) + '.png')
